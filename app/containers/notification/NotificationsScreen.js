...
import { blackVariant, white } from "../util/colors";
...
class NotificationsScreen extends BaseComponent {
  ...
  selfComponentDidMount() {
    ...
    getData("header", (value) => {
      this.props
        .getNotificationListing(value, page, size)
        .then(() => {
  ...
  _renderItem = ({ item }) => {
    ...
    if (item.is_seen == true) backgroundColor = "#fff";
        ...
        >
          ...
          >
            ...
            <View style={{ marginLeft: 0 }}>
              ...
              >
                {this.getRelativeTime(item.created_at)}
              </Text>
            </View>
          </View>
        </TouchableOpacity>
  ...
  _progressLoader = ({}) => {
    if (this.state.pagingLoader) {
      return <ActivityIndicator size="large" color="#67BAF5" />;
    } else {
      return null;
    }
  };
